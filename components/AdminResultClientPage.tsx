...
export default function AdminResultClientPage({ initialRequests }: { initialRequests: AdminRequest[] }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/admin/requests/process/result', {
        method: 'POST',
    ...
    return (
      <div className="space-y-4 text-sm text-gray-700">
        ...
        <div className="bg-pink-50 p-3 rounded border border-pink-200">
          <p className="font-bold text-pink-900 border-b border-pink-200 pb-1 mb-2">Exam Info</p>
          <div className="grid grid-cols-2 gap-4">
             <div>
                <p className="text-xs text-gray-500 uppercase">Reg Number</p>
                <p className="font-mono font-bold">{d.regNumber}</p>
             </div>
             <div>
                <p className="text-xs text-gray-500 uppercase">Year</p>
                <p className="font-bold">{d.year || 'N/A'}</p>
             </div>
          </div>
        </div>

        {/* Specific Fields per Exam Type */}
        <div className="border rounded-lg p-3">
           <p className="font-bold text-gray-900 mb-2">Additional Details</p>
           
           {req.service.name.includes('WAEC') && (
           ...
           {req.service.name.includes('NECO') && (
             <div className="grid grid-cols-1 gap-2">
               <p><strong>Candidate Name:</strong> {d.name}</p>
               <p><strong>Phone:</strong> {d.phone}</p>
               <p><strong>Network:</strong> {d.network}</p>
             </div>
           )}

           {req.service.name.includes('NABTEB') && (
             <div className="grid grid-cols-2 gap-2">
               <p><strong>Type:</strong> {d.examType}</p>
               <p><strong>Serial:</strong> {d.serial}</p>
             </div>
           )}
        </div>
        ...
        <div className="p-3 bg-gray-50 rounded text-xs text-gray-500">
           Requested by: {req.user.firstName} {req.user.lastName} ({req.user.email})
        </div>
      </div>
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-200">
        <div className="relative flex-1 max-w-md">
           ...
           <input 
             type="text" placeholder="Search Reg No, Name..." 
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
          {['ALL', 'PENDING', 'PROCESSING', 'COMPLETED', 'FAILED'].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)} className={`px-3 py-1.5 text-xs font-bold rounded-md ${filterStatus === s ? 'bg-white shadow text-pink-600' : 'text-gray-500'}`}>{s}</button>
          ))}
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Reg No</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredRequests.map((req) => (
                <tr key={req.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-xs font-medium text-gray-800 font-mono">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative my-8 animate-in fade-in zoom-in-95 duration-200">
             <div className="flex justify-between items-center border-b border-gray-200 p-5">
               <h3 className="text-xl font-bold text-gray-900">Result Request Details</h3>
            ...
            <div className="space-y-4">
              {actionType === 'COMPLETED' && (
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-gray-700">Upload Result (PDF)</label>
