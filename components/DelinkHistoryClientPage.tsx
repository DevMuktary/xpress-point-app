...
type Props = {
  initialRequests: DelinkRequest[];
};

export default function DelinkHistoryClientPage({ initialRequests }: Props) {
  ...
  const filteredRequests = useMemo(() => {
    return allRequests.filter(req => {
      const matchesSearch = req.nin.includes(searchTerm);
  ...
  // Helper for the "smart" status/action display
  const renderStatusCard = (request: DelinkRequest) => {
    switch (request.status) {
      case 'COMPLETED':
        return (
          <div className="text-sm text-center text-green-800 p-2 rounded-lg bg-green-50 border border-green-200">
            <p className="font-semibold">Success! Your NIN is delinked.</p>
          </div>
        );
      case 'FAILED':
        return (
      ...
      <div className="space-y-4">
        {!isLoading && filteredRequests.length === 0 && (
          <div className="py-12 text-center text-gray-500">
            ...
            <p className="text-sm">No delink requests match your filters.</p>
          </div>
        )}
