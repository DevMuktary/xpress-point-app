...
export default function BvnNibssHistoryClientPage({ initialRequests }: { initialRequests: RequestItem[] }) {
  const [requests] = useState(initialRequests);
  const [searchTerm, setSearchTerm] = useState('');

  const filteredRequests = requests.filter(req => {
    const d = req.formData || {};
    const searchStr = `${d.ticketId || ''} ${req.status}`.toLowerCase();
  ...
  return (
    <div className="space-y-6">
      <div className="relative">
        ...
        <input
          type="text" placeholder="Search by Ticket ID..." value={searchTerm}
            ...
            return (
              <div key={req.id} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                ...
                <div className="text-sm text-gray-700 bg-gray-50 p-3 rounded-lg mb-3">
                  <p><strong>Ticket ID:</strong> {d.ticketId || 'N/A'}</p>
                </div>
