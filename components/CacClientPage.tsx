...
// --- The Main Component ---
// We must accept the props from the server page
type Props = {
  serviceId: string;
  serviceName: string;
  serviceFee: number;
...
export default function CacClientPage({ serviceId, serviceName, serviceFee }: Props) {
  ...
  const handleOpenConfirmModal = (e: React.FormEvent) => {
    ...
    if (serviceId === 'CAC_REG_BN' && (!passportUrl || !signatureUrl || !ninSlipUrl)) {
  ...
  const handleFinalSubmit = async () => {
    ...
    } else if (serviceId === 'CAC_DOC_RETRIEVAL') {
      formData = {
        docType: 'N/A', // Placeholder, we removed this logic
        fullBizName: propFirstName, // Re-mapping from your old code
        bizNumber: propNin,
      ...
      <div className="rounded-2xl bg-white p-6 shadow-lg">
        <form onSubmit={handleOpenConfirmModal} className="space-y-6">
          ...
          {serviceId === 'CAC_DOC_RETRIEVAL' && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900">Enter Required Details</h3>
              <DataInput label="Full Business Name*" id="fullBizName" value={propFirstName} onChange={setPropFirstName} Icon={BriefcaseIcon} />
              <DataInput label="Business Number (BN/RC)*" id="bizNumber" value={propNin} onChange={setPropNin} Icon={BuildingOfficeIcon} />
          ...
          {serviceId && (
            <div className="border-t border-gray-200 pt-6 space-y-6">
              ...
              >
                {isSubmitting ? 'Submitting...' : `Submit Request (Fee: ₦${serviceFee})`}
              </button>
            </div>
          )}
        </form>
