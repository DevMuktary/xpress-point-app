...
import { 
  CheckCircleIcon, XCircleIcon, ClipboardDocumentIcon, ArrowPathIcon, 
  BanknotesIcon, ClockIcon, PencilSquareIcon
} from '@heroicons/react/24/outline';

export default function AdminPayoutsClientPage({ initialPayouts, payoutHistory, accountChanges }: any) {
  const [activeTab, setActiveTab] = useState('PENDING'); // 'PENDING', 'HISTORY', 'CHANGES'
  const [payouts, setPayouts] = useState(initialPayouts);
  const [changes, setChanges] = useState(accountChanges);
  ...
  const handleProcessPayout = async (id: string, action: 'APPROVE' | 'REJECT') => {
    ...
    try {
      ...
      setPayouts(prev => prev.filter((p: any) => p.id !== id));
      alert("Success!");
      window.location.reload(); // Reload to update history tab
    } catch (error) { alert("Failed."); } finally { setProcessingId(null); }
  };
  ...
  const handleProcessChange = async (id: string, action: 'APPROVE' | 'REJECT') => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed");
      setChanges(prev => prev.filter((c: any) => c.id !== id));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 border-b border-gray-200">
        <button onClick={() => setActiveTab('PENDING')} className={`pb-3 px-4 font-bold text-sm border-b-2 transition-all ${activeTab === 'PENDING' ? 'border-green-600 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
          Pending Payouts ({payouts.length})
        </button>
        <button onClick={() => setActiveTab('CHANGES')} className={`pb-3 px-4 font-bold text-sm border-b-2 transition-all ${activeTab === 'CHANGES' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
          Account Updates ({changes.length})
        </button>
        <button onClick={() => setActiveTab('HISTORY')} className={`pb-3 px-4 font-bold text-sm border-b-2 transition-all ${activeTab === 'HISTORY' ? 'border-gray-600 text-gray-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
          Payout History
        </button>
      </div>
            ...
            payouts.map((payout: any) => (
              <div key={payout.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 bg-gray-50 p-4 rounded-lg text-sm">
                  ...
                  <div className="flex gap-2"><strong>Acc:</strong> {payout.user.accountNumber} <button onClick={() => copyToClipboard(payout.user.accountNumber)}><ClipboardDocumentIcon className="h-4 w-4 text-blue-500"/></button></div>
                  <p><strong>Name:</strong> {payout.user.accountName}</p>
                </div>
                <div className="flex flex-col gap-2 w-full md:w-32">
                  <button onClick={() => handleProcessPayout(payout.id, 'APPROVE')} disabled={!!processingId} className="py-2 bg-green-600 text-white rounded font-bold text-sm">Pay</button>
                  <button onClick={() => handleProcessPayout(payout.id, 'REJECT')} disabled={!!processingId} className="py-2 bg-red-50 text-red-600 border border-red-200 rounded font-bold text-sm">Reject</button>
                </div>
              </div>
            ))
          )}
          ...
          {changes.length === 0 ? (
            ...
            changes.map((change: any) => (
              <div key={change.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row gap-6">
                ...
                <div className="flex flex-col gap-2 w-full md:w-32">
                  <button onClick={() => handleProcessChange(change.id, 'APPROVE')} disabled={!!processingId} className="py-2 bg-blue-600 text-white rounded font-bold text-sm">Approve</button>
                  <button onClick={() => handleProcessChange(change.id, 'REJECT')} disabled={!!processingId} className="py-2 bg-red-50 text-red-600 border border-red-200 rounded font-bold text-sm">Reject</button>
                </div>
              </div>
            ))
          )}
