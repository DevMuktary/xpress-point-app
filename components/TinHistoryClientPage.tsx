...
import { TinRequest, Service, RequestStatus } from '@prisma/client';
import { 
  CheckCircleIcon,
  XCircleIcon,
  ArrowPathIcon,
...
type TinHistoryRequest = TinRequest & {
  service: { name: string };
};

// Define the props to receive the initial data from the server
type Props = {
  initialRequests: TinHistoryRequest[];
};

export default function TinHistoryClientPage({ initialRequests }: Props) {
  ...
  const fetchHistory = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/services/tin/history');
      if (!res.ok) {
  ...
  const filteredRequests = useMemo(() => {
    return allRequests.filter(req => {
      ...
      const searchData = formData.nin || formData.bizName || formData.bvnOrNin || '';
  ...
  const renderActionButton = (request: TinHistoryRequest) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search by NIN, BVN, or Business Name..."
            value={searchTerm}
      ...
      <div className="space-y-4">
        {!isLoading && filteredRequests.length === 0 && (
          <div className="py-12 text-center text-gray-500">
            ...
            <p className="text-sm">No TIN requests match your filters.</p>
          </div>
        )}
        ...
        {filteredRequests.map((request) => {
          ...
          const identifier = formData.nin || formData.bizName || formData.bvnOrNin;
