...
// (All the same imports as the WAEC file...)
import { 
  CheckCircleIcon,
...
// (All the same helper components: CopyButton, PinCard, DataInput, ModTypeButton)
...
export default function JambPinClientPage() { // <-- Corrected component name
  
  // --- "World-Class" Service IDs & Fees ---
  type ServiceID = 'JAMB_UTME_PIN' | 'JAMB_DE_PIN';
  const [serviceId, setServiceId] = useState<ServiceID | null>(null);
  
  const fee = useMemo(() => {
    if (serviceId === 'JAMB_UTME_PIN') return 500;
    if (serviceId === 'JAMB_DE_PIN') return 500;
    return 0;
  }, [serviceId]);
      ...
      <div className="rounded-2xl bg-white p-6 shadow-lg mt-6">
        ...
        {!isHistoryLoading && requests.length === 0 && (
          <div className="py-8 text-center text-gray-500">
            ...
            <p className="text-sm">You have not purchased any JAMB pins yet.</p>
          </div>
        )}
              ...
              <div className="flex justify-between items-center mb-2">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {request.serviceId === 'JAMB_DE_PIN' ? 'Direct Entry' : 'UTME'}
                  </p>
                </div>
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl">
            ...
            <div className="p-6">
              <p className="text-center text-gray-600">
                Are you sure you want to purchase {quantity} {serviceId === 'JAMB_DE_PIN' ? 'Direct Entry' : 'UTME'} PIN(s) for {phoneNumber}? This action is irreversible.
              </p>
